...
//  CAN_BTR_LBKM | CAN_BTR_BRP(6) | CAN_BTR_TS1(8) | CAN_BTR_TS2(1) | CAN_BTR_SJW(0)
...
static CANConfig canCfg250K = {
  0,
  CAN_BTR_BRP(21) | CAN_BTR_TS1(12) | CAN_BTR_TS2(3) | CAN_BTR_SJW(0)
};
...
static void cmdtestDecToBin(BaseSequentialStream *chp, int argc, char *argv[]);
...
char* DecToBin(int bit_size, int input, char* output) {
    char temp[bit_size + 1];
    if (bit_size < MIN_BIT_SIZE) {
        return "Error!!! Bit_size is less than MIN_BIT_SIZE";
    }

    if (bit_size > MAX_BIT_SIZE) {
        return "Error!!! Bit_size is larger than MAX_BIT_SIZE";
    }
...
static void cmdtestDecToBin(BaseSequentialStream *chp, int argc, char *argv[]){
     //argv[1] BitSize, argv[2] Data
  
        if (argc < 2 || argv == NULL) { //Check if argv is NULL or argc is less than 2
            chprintf(chp, "Error!!! Insufficient or invalid arguments");
            chThdSleepMilliseconds(10);
            return;
        }
  
        int BitSize = atoi(argv[0]);
        chprintf(chp, "BitSize = %d\n", BitSize);
        chThdSleepMilliseconds(10
        ...
        bool flag = false; // decide is there any error happened
        
        if (BitSize < 0) { // avoid bitsize is negative
            flag = true;
            chprintf(chp, "Error!!! Bit_size cannot be negative");
            chThdSleepMilliseconds(10);
        }
        
        if (Data < 0) { // avoid data is negative
            flag = true;
            chprintf(chp, "Error!!! Data cannot be negative");
            chThdSleepMilliseconds(10);
        }
        
        if (BitSize < MIN_BIT_SIZE) { // avoid occupying too much CPU
            flag = true;
            chprintf(chp, "Error!!! Bit_size is less than MIN_BIT_SIZE");
            chThdSleepMilliseconds(10);
        }
        
        if (BitSize > MAX_BIT_SIZE) { // avoid occupying too much CPU
            flag = true;
            chprintf(chp, "Error!!! Bit_size is larger than MAX_BIT_SIZE");
            chThdSleepMilliseconds(10);
        }
        
        
        // Check if BitSize is less than binary representation of Data
        int binaryDataLength = floor(log2(Data)) + 1;
        
        if (BitSize < binaryDataLength) {
            flag = true;
            chprintf(chp, "Error!!! Bit_size is less than binary representation of Data");
            chThdSleepMilliseconds(10);
        }
        ...
        sprintf(temp,"%x",Data);
        chprintf(chp, "this = %s\n", temp);//added
        chprintf(chp, "strlen(temp) = %d\n", strlen(temp));
        char out[BitSize];
        
        int outputlength = strlen(temp);
        for (int i = 0; i < BitSize - outputlength; i++) {
          out[i] = '0';
        }
        int L = BitSize - outputlength;
        for (int i = L, j = 0; i < L + outputlength; i++, j++) {
          out[i] = temp[j];
        }    
        chprintf(chp, "result = %s\n", out);
        
       
        //for (int i = 0; i < BitSize - strlen(temp); i++){
        //    out[i] = '7';
        //}
        //chprintf(chp, "result = %s\n", out);
        //chThdSleepMilliseconds(10);
